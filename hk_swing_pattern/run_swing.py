...
def write_excel(df: pd.DataFrame, asof: str, out: Path, col_order: list = None):
    ...
    info = [
        ...
        ("  SOS_orig (原版 SoS): 近3日, 60日区间pos<=0.50 + 放量>=1.5x + (阳线body>=3%&range>=1.5x&close_pos>=0.70 或 十字星)。纯底部反强, 无均线纠缠", 10, False),
        ("  SOS (新版 SoS): 位置门 = A OR B。A: 6个月pos<=0.50(6月数据不足回退3个月); B: 近3日 4均线(5/10/15/20)纠缠 max/min-1<5%。强度同原版(放量+阳线/十字星)。捕获2328式纠缠平台突破、9980式次新股底部反强", 10, False),
